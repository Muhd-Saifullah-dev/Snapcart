...
const slides = [
    {
        id: 1,
        icon: (
            <Leaf className="w-20 h-20 sm:h-28 text-green-400 drop-shadow-lg" />
        ),
        title: 'Fresh Organic Groceries 🥦',
        subtitle:
            'Farm-fresh fruits, vegetable, and daily essentials delivered to you',
        btnText: 'Shop Now',
    ...
    },
    {
        id: 2,
        icon: (
            <Truck className="w-20 h-20 sm:h-28 text-amber-400 drop-shadow-lg" />
        ),
        title: 'Fast Reliable Delivery 🚚',
        subtitle:
            'We ensure your groceries reach your doorstep delivered to you',
        btnText: 'Order Now',
    ...
    },
    {
        ...
        subtitle: 'Easy and seamless online groceries shopping experience',
        btnText: 'Get Started',
        ...
        <div className="relative w-[98%] mx-auto mt-32 h-[80vh] rounded-3xl overflow-hidden shadow-2xl">
            ...
            <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-3">
                {slides.map((_, index) => (
                    <button
                        key={index}
                        className={`w-3 h-3 rounded-xl transition-all ${index === current ? 'bg-white w-6 ' : 'bg-white/50'}`}
                    ></button>
                ))}
            </div>
        </div>
