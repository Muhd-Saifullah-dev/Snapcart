...
const slides = [
  ...
  {
    id: 2,
    icon: <Truck className="w-20 h-20 sm:h-28 text-amber-400 drop-shadow-lg" />,
    title: 'Fast Reliable Delivery 🚚',
    subtitle: 'We ensure your groceries reach your doorstep delivered to you',
    btnText: 'Order Now',
...
function HeroSection() {
  ...
  return (
    ...
    {slides.map((_,index)=>(
        <button key={index} className={`w-3 h-3 rounded-xl transition-all ${index===current ? "bg-white w-6 ":"bg-white/50"}`}></button>
    ))}
    </div>
