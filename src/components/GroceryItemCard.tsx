...
function GroceryItemCard({ item }: { item: IGrocery }) {
    ...
    <Image src={item.image} fill alt={item.name} sizes='(max-width:758px) 100vw,25vw'
    ...
    <div className='absolute inset-0 bg-linear-to-t from-black/10 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300'/>
        ...
        <div className='p-4 flex flex-col flex-1'>
            ...
            <motion.button className='mt-4 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white rounded-full py-2 text-sm font-bold transition-all'
            whileTap={{scale:0.96}}>
