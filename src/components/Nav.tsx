...
function Nav({ user }: { user: IUSER }) {
  ...
  const sidebar=menuOpen?createPortal(
    <AnimatePresence>
      ...
      className='fixed top-0 left-0 h-full w-[75%] sm:w-[60%] z-999 bg-linear-to-b from-green-800/90 via-green-700/80 to-green-900/90 backdrop-blur-xl border-r border-green-400/20 shadow-[0_0_15px_-10px_rgba(0,255,100,0.3)] flex flex-col p-6 text-white' 
      >
        ...
        <div className='flex items-center gap-3 p-3 mt-3 rounded-xl bg-white/10 hover:bg-white/15 transition-all shadow-inner'>
          <div className='relative w-12 h-12 rounded-full overflow-hidden border-2 border-green-400/60 shadow-lg'> {user.image ? (
              <Image
                src={user.image}
                alt="user"
                fill
  ...
  return (
    <div className="w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-linear-to-r from-green-500 to-green-700 rounded-xl shadow-lg shadow-black/30 flex justify-between  items-center h-20 px-4 md:px-8 z-50 ">
      <Link
        href={'/'}
        ...
        {user.role === 'admin' && (
          <>
            <div className="hidden md:flex items-center gap-4">
              <Link
                href={''}
                className="flex items-center gap-2 text-green-700 bg-white font-semibold px-4 py-2 rounded-full hover:bg-green-100 transition-all"
              >
                <PlusCircle className="w-5 h-5" /> Add Grocery
              </Link>
              <Link
